  ...
  async createTerminal(
    ...
    if (shouldCreateInBackground) {
      ...
      try {
        ...
        const effectiveClaudeAgentTeamsMode = dependencies.inferCapturedClaudeAgentTeamsMode(
          launchOpts.launchConfig,
          claudeAgentTeamsSourceCommand,
          claudeAgentTeamsMode
        )
        let agentTeamsPlan:
          | Awaited<ReturnType<typeof dependencies.buildClaudeAgentTeamsLaunchPlan>>
          | undefined
        try {
          agentTeamsPlan = adoptedBeforeLaunch
            ? undefined
            : await dependencies.buildClaudeAgentTeamsLaunchPlan({
            ...
            ? {
                ...
                agentCommand: launchOpts.launchConfig.agentCommand
                  ? effectiveClaudeAgentTeamsMode === 'in-process' || process.platform === 'win32'
                    ? dependencies.addClaudeTeammateModeInProcess(
                        launchOpts.launchConfig.agentCommand
                      )
                    : dependencies.addClaudeTeammateModeAuto(launchOpts.launchConfig.agentCommand)
                  : agentTeamsPlan.command,
                agentEnv: { ...launchOpts.launchConfig.agentEnv, ...agentTeamsPlan.env }
              }
            : launchOpts.launchConfig
